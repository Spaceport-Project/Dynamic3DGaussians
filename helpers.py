...
def setup_camera(w, h, k, w2c, active_sh_degree =0, near=0.01, far=100, scale=1, bg=torch.tensor([0, 0, 0])):
...
def setup_camera_v1(w, h, k, w2c, active_sh_degree =0, near=0.01, far=100, scale=1, bg=torch.tensor([0, 0, 0])):
    fx, fy, cx, cy = k[0][0], k[1][1], k[0][2], k[1][2]
    w2c = torch.tensor(w2c).cuda().float()
    cam_center = torch.inverse(w2c)[:3, 3]
    w2c = w2c.unsqueeze(0).transpose(1, 2)
...
def get_features(params, active_mask=None):
    features_dc = params['f_dc']
    features_rest = params['f_rest']
    if active_mask is not None:
        features_dc = features_dc[active_mask]
        features_rest = features_rest[active_mask]
...
def params2rendervarseg(params):
    rendervar = {
        'means3D': params['means3D'],
        'colors_precomp': None,
        'rotations': torch.nn.functional.normalize(params['unnorm_rotations']),
    ...
    return rendervar
def params2rendervarshs(params, active_mask=None):
    shs = get_features(params, active_mask)
    if active_mask is not None:
        rendervar = {
            'means3D': params['means3D'][active_mask],
            # 'colors_precomp': torch.sigmoid(params['rgb_colors']),
            'shs': shs, 
            'rotations': torch.nn.functional.normalize(params['unnorm_rotations'][active_mask]),
            'opacities': torch.sigmoid(params['logit_opacities'][active_mask]),
            'scales': torch.exp(params['log_scales'][active_mask]),
            'means2D': torch.zeros_like(params['means3D'][active_mask], requires_grad=True, device="cuda") + 0
        }
    else:
        rendervar = {
            'means3D': params['means3D'],
...
def params2rendervarshsaccel(params, active_mask=None):
    ...
    try:
        if active_mask is not None:
            rendervar = {
                'means3D': params['means3D'][active_mask],
                'dc':params['f_dc'][active_mask],
                'shs': params['f_rest'][active_mask], 
                'rotations': torch.nn.functional.normalize(params['unnorm_rotations'][active_mask]),
                'opacities': torch.sigmoid(params['logit_opacities'][active_mask]),
                'scales': torch.exp(params['log_scales'][active_mask]),
                'means2D': torch.zeros_like(params['means3D'][active_mask], requires_grad=True, device="cuda") + 0
        ...
        print("Empty, NaN or Inf in rotations!")
        rendervar = None
    return rendervar
...
def params2cpu(params, is_initial_timestep):
    if is_initial_timestep:
        res = {k: v.detach().cpu().contiguous().numpy() for k, v in params.items() if k not in ['cam_m', 'cam_c', 'seg_colors' ]}
    else:
        res = {k: v.detach().cpu().contiguous().numpy() for k, v in params.items() if
               k in ['means3D', 'rgb_colors', 'unnorm_rotations']}
    return res
# def params2cpushs(params):
...
def save_params_single(output_params, seq, exp, ind):
    ...
    os.makedirs(f"./output/{seq}/{exp}", exist_ok=True)
    np.savez(f"./output/{seq}/{exp}/params_{ind}", **to_save)

def save_params_to_ply(output_params, seq, exp, ind):
    
    num_vertices = output_params["means3D"].shape[0]
    

    vertex_data = np.empty(num_vertices, dtype=vertex_dtype)

    vertex_data['x'] = output_params["means3D"][:, 0]
    vertex_data['y'] = output_params["means3D"][:, 1]
    vertex_data['z'] = output_params["means3D"][:, 2]
    
    rgb_colors = output_params["rgb_colors"] #sigmoid(output_params["rgb_colors"])
    rgb_colors = RGB2SH(rgb_colors)
    # Assuming rgb_colors are floats, assign to f_dc_0, f_dc_1, f_dc_2
    vertex_data['f_dc_0'] = rgb_colors[:, 0]
    vertex_data['f_dc_1'] = rgb_colors[:, 1]
    vertex_data['f_dc_2'] = rgb_colors[:, 2]

    vertex_data['opacity'] = output_params["logit_opacities"].reshape(-1,)

    vertex_data['scale_0'] = output_params["log_scales"][:, 0]
    vertex_data['scale_1'] = output_params["log_scales"][:, 1]
    vertex_data['scale_2'] = output_params["log_scales"][:, 2]

    rotations = normalize(output_params["unnorm_rotations"])
    ...
    os.makedirs(f"./output/{exp}/{seq}", exist_ok=True)
    output_ply_path = os.path.join(f"./output/{exp}/{seq}", f"params_{ind:03d}.ply" )
    output_compr_ply_path = os.path.join(f"./output/{exp}/{seq}", f"params_{ind:03d}.compressed.ply" )
    PlyData([vertex_element], text=False).write(output_ply_path)
    cmd = ['splat-transform', '-w', output_ply_path,  output_compr_ply_path]
    subprocess.run(cmd, check=True)


def construct_list_of_attributes(params):
    # l = ['x', 'y', 'z',  'nx', 'ny', 'nz']
    l = ['x', 'y', 'z']

    # All channels except the 3 DC
    for i in range(params["f_dc"].shape[1]*params["f_dc"].shape[2]):
        l.append('f_dc_{}'.format(i))
    for i in range(params["f_rest"].shape[1]*params["f_rest"].shape[2]):
        l.append('f_rest_{}'.format(i))
    l.append('opacity')
    for i in range(params["log_scales"].shape[1]):
        l.append('scale_{}'.format(i))
    for i in range(params["unnorm_rotations"].shape[1]):
        l.append('rot_{}'.format(i))
    return l



def save_params_to_ply_shs(params, mask, output_path, ind, file_suffix=""):
    
    xyz = params["means3D"][mask].detach().cpu().numpy()
    f_dc = params["f_dc"][mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    f_rest = params["f_rest"][mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    opacities = params["logit_opacities"][mask].detach().cpu().numpy()
    scale = params["log_scales"][mask].detach().cpu().numpy()
    rotation = normalize(params["unnorm_rotations"][mask].detach().cpu().numpy())

    

    dtype_full = [(attribute, 'f4') for attribute in construct_list_of_attributes(params)]
