...
class RenderViewers():
    ...
    def render_images(self):
        ...
        while self.running.value:
            ...
            # print(self.client.camera._predicted_cams)
            # print("cam state", self.client.camera._state)
            # R_S03_pred_cams = self.client.camera._predicted_cams.wxyz[:3]
            # R_pred_cams = [ R.as_matrix() for R in R_S03_pred_cams ]
            ...
            # Calculate camera matrices
                ...
                c2w_1 = np.vstack((np.concatenate((R, T[:,None]), axis=1),[0,0,0,1]))
            ...
            c2w_pred_cams = []
            # c2w_pred_cams.append(c2w)
            num_pred_cams = 4
            if self.client.camera._predicted_cams and len(self.client.camera._predicted_cams.wxyz) >= num_pred_cams:
                # t1=time.time()
                for ii, quat in enumerate(self.client.camera._predicted_cams.wxyz):
                    R = tf.SO3(quat).as_matrix()
                    T = self.client.camera._predicted_cams.position[ii]
                    c2w_temp = np.vstack((np.concatenate((R, T[:,None]), axis=1),[0,0,0,1]))
                    if ii == len(self.client.camera._predicted_cams.wxyz) -1 :
                        c2w_1 =c2w_temp.copy()
                        
                    c2w_pred_cams.append(c2w_temp)


                # t2 = time.time()
                # print(t2-t1)
                # w2c = np.linalg.inv(c2w)
                self.client.camera._predicted_cams = {}
                
            else:
                num_pred_cams = 1
                c2w_pred_cams.append(c2w_1)
            print("len:", len(c2w_pred_cams))
            # try:

            #     scenes_data = [self.scene_data.get(timeout=2)  for _ in range(num_pred_cams+1)]  
            ...
            for ii, c2w in enumerate(c2w_pred_cams):
                t0 = time.time()
                try:
    ...
    @staticmethod
    def render_process_function(render_input_queue, render_output_queue, bg_scene_data_info, max_sh, w, h, k, near, far, render_event, render_ready_event, running):
        ...
        try:
            while running.value:
                try:
                    ...
                    try:
                        item = render_input_queue.get(timeout=0.1)
                    except queue.Empty:
                    ...
                    # print("c2w:", c2w) 
                    # print('c2w_pred_cams:',c2w_pred_cams)
                    w2c = np.linalg.inv(c2w)
                    ...
                    with torch.no_grad():
                        ...
                        result = im.permute(1,2,0).contiguous()
                        result = (result.clamp(0,1)*255).to(torch.uint8)
                        
                        try:  
                            render_output_queue.put(result, timeout=0.1)  
                        except queue.Full:  
...
if __name__ == "__main__":
    ...
    viewer = Viewer(seq=sequence, exp=exp_name, pc_path=pc_path, port=8093, title="360° Demo9 Spaceport", w=1920, h=1080, max_sh_degree=0)
