...
def init_worker(tensor_info, gpu_id):
    """Initialize worker with shared memory tensors"""
    ...
    for tensor_name, info in tensor_info.items():
        ...
        WORKER_SHARED_MEMORIES[tensor_name] = shm
    
    print(f"Worker initialized on GPU {gpu_id} with shared tensors: {list(CACHED_TENSOR_DICT.keys())}")

def high_frequency_renderer(client_id, tensor_operations, render_config, result_queue, control_queue):
    """High-frequency renderer producing frames every 30ms (33.33 FPS)"""
    global CACHED_TENSOR_DICT, WORKER_GPU_ID
    
    print(f"🚀 Started high-frequency rendering for client {client_id} on GPU {WORKER_GPU_ID}")
    ...
    try:
        while True:
            ...
            for tensor_name, slice_info in tensor_operations:
                ...
                shared_tensor = CACHED_TENSOR_DICT[tensor_name]
            ...
            # Create frame result with additional metadata for threading
            frame_result = {
                'client_id': client_id,
                ...
                'metadata': {
                    ...
                    'memory_usage': torch.cuda.memory_allocated(WORKER_GPU_ID) if torch.cuda.is_available() else 0,
        ...
        print(f"❌ Error in high-frequency renderer for client {client_id}: {e}")
        error_result = {
            'client_id': client_id,
            'error': str(e),
            'timestamp': time.time(),
        ...
        print(f"🏁 Client {client_id} completed: {frame_count} frames in {total_time:.1f}s "
              f"(Actual FPS: {actual_fps:.1f}, Target: 33.33)")

def fast_render_frame(gpu_tensor, tensor_name, render_config, client_id, frame_number, output_tensor):
...
class ClientJobProcessor:
    ...
    def _execute_client_job(self):
        ...
        This is where you put your actual job logic
        """
...
class ThreadedRenderingManager:
    """Enhanced rendering manager with per-client background job processing"""
    ...
    def _initialize_executors(self):
        """Initialize executors optimized for high-frequency rendering"""
        ...
        print(f"Initialized {self.num_gpus} GPU executors with {self.workers_per_gpu} workers each")
    
    def connect_client(self, client_id, tensor_operations, render_config=None, job_config=None):
        """
        Connect client for high-frequency rendering with background jobs
        
        Args:
            client_id: Unique client identifier
            tensor_operations: List of (tensor_name, slice_info)
            render_config: Rendering configuration
            job_config: Background job configuration
        
        Returns:
            result_queue: Queue to retrieve frames every 30ms
        ...
        with self.lock:
            ...
            print(f"🚀 Connected client {client_id} to GPU {gpu_id} with background jobs")
    ...
    def get_comprehensive_status(self):
        """Get comprehensive status including rendering and job processing"""
        with self.lock:
            status = {
    ...
    def shutdown(self):
        """Shutdown all clients, jobs, and executors"""
        print("Shutting down threaded rendering manager...")
        
        # Disconnect all clients (this will stop their jobs too)
        with self.lock:
            client_ids = list(self.active_clients.keys())
        ...
        print("Threaded rendering manager shut down")

def main():
    """Example of threaded client processing with rendering and background jobs"""
    mp.set_start_method("spawn", force=True)
    
    # Shared tensors
    shared_tensors = {
        "scene_data": torch.randn(20000, 128),
        "lighting": torch.randn(10000, 64),
        "materials": torch.randn(5000, 256),
    }
    
    tensor_manager = ZeroCopyTensorManager(shared_tensors)
    
    try:
        # Create threaded rendering manager
        render_manager = ThreadedRenderingManager(
            tensor_manager,
            workers_per_gpu=4,
        ...
        for i in range(4):
            ...
            render_config = {
                ...
                'style': ['smooth_wave', 'pulse', 'rotate', 'default'][i],
                'batch_size': 1
            }
            ...
            if success:
                ...
                print(f"✅ Connected {client_id} with {job_types[i]} jobs")
        ...
        while time.time() - start_time < 15:
            ...
            if current_time - last_stats_time >= 5.0:
                ...
                for client_id, details in status['client_details'].items():
                    ...
                    print(f"   {client_id} (GPU {details['gpu_id']}):")
        ...
        final_status = render_manager.get_comprehensive_status()
        print(f"\n📈 Final Comprehensive Status:")
        for client_id, details in final_status['client_details'].items():
            print(f"   {client_id}: Job type: {details['job_config']['job_type']}, "
                  f"Style: {details['render_config']['style']}")
