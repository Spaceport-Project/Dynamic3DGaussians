...
def get_dataset(t, md, seq, iters, scale=1, bg=torch.tensor([0, 0, 0])):
    ...
    for c in range(len(md['fn'][t])):
        ...
        width, height = image.size
        # resized_image = image.resize((int(width/scale), int(height/scale)))
        resized_image = image.resize((2133, 1519))
        resized_image = np.array(resized_image)
        resized_image = torch.tensor(resized_image).float().cuda().permute(2, 0, 1) / 255 # torch.from_numpy(np.array(resized_image)) / 255.0
        # if len(resized_image.shape) == 3:
        #     resized_image.cuda().permute(2, 0, 1)
        ...
        # resized_seg =  seg.resize((int(width/scale), int(height/scale)))
        resized_seg =  seg.resize((2133, 1519))
...
def get_loss(params, curr_data, variables, is_initial_timestep, t):
    ...
    loss_weights = {'im': 1.0, 'seg': 2.0, 'rigid': 4.0, 'rot': 4.0, 'iso': 2.0, 'floor': 2.0, 'bg': 20.0,
                    'soft_col_cons': 0.01}
...
def initialize_per_timestep(params, variables, optimizer):
    ...
    variables["prev_col"] = params['rgb_colors'].clone().detach()
...
if __name__ == "__main__":
    ...
    for sequence in ["2025-05-20_18-24-20_hamit_burak_1_new1"]:

    # exp_name = "2025-05-20_18-28-33_hamit_burak_2_half_all_1_enhanced_test1"
    # for sequence in ["2025-05-20_18-28-33_hamit_burak_2_half_new1"]:
