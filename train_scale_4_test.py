...
def get_loss(params, curr_data, variables, is_initial_timestep):
    ...
    current_psnr = calc_psnr(im- curr_data['im'] )
    
    # Track camera performance
    if 'cam_performance' not in variables:
        variables['cam_performance'] = {}
    if curr_id not in variables['cam_performance']:
        variables['cam_performance'][curr_id] = []

    variables['cam_performance'][curr_id].append(current_psnr.item())

    # Keep only recent history
    if len(variables['cam_performance'][curr_id]) > 50:
        variables['cam_performance'][curr_id] = variables['cam_performance'][curr_id][-50:]

    # Calculate adaptive weight
    if len(variables['cam_performance']) > 1 and len(variables['cam_performance'][curr_id]) > 5:
        # Get average PSNR for this camera
        cam_avg_psnr = sum(variables['cam_performance'][curr_id][-10:]) / min(10, len(variables['cam_performance'][curr_id]))

        # Get average PSNR across all cameras
        all_avgs = []
        for cam_id, history in variables['cam_performance'].items():
            if len(history) > 5:
                all_avgs.append(sum(history[-10:]) / min(10, len(history)))

        if len(all_avgs) > 1:
            global_avg_psnr = sum(all_avgs) / len(all_avgs)
            # Increase weight for underperforming cameras
            psnr_gap = max(0, global_avg_psnr - cam_avg_psnr)
            adaptive_weight = 1.0 + (psnr_gap / 5.0)  # Increase weight proportionally
        else:
            adaptive_weight = 1.0
    else:
        adaptive_weight = 1.0

    # Apply adaptive weighting to image loss
    losses['im'] = adaptive_weight * (0.8 * l1_loss_v1(im, curr_data['im']) + 0.2 * (1.0 - calc_ssim(im, curr_data['im'])))
