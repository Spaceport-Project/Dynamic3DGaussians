...
class WeightedLoss(torch.nn.Module):
    def __init__(self, num_losses=6):
        super().__init__()
        # Initialize unconstrained weights (log scale)
        self.log_weights = torch.nn.Parameter(torch.zeros(num_losses).cuda().float().requires_grad_(True))

    # def set_log_weights(self, size):
    #     if self.log_weights.shape[0] < size:
    #         self.log_weights = torch.nn.Parameter(torch.zeros(num_losses).cuda().float())
    def forward(self, losses):
        # losses: list or tensor of 7 loss values
        weights = torch.exp(self.log_weights)  # ensure weights > 0
        weights = weights * torch.tensor([1.0, 3.0, 4.0, 4.0, 2.0, 0.02, 0.05], device=weights.device)  
...
def initialize_optimizer(params, variables, criterion):
    lrs = {
    ...
    weight_groups = [{'params': [criterion.log_weights], 'name': 'w', 'lr': lrs_w['w']}]
    return torch.optim.Adam(param_groups + weight_groups, lr=0.0, eps=1e-15)
...
def get_loss(params, optimizer, curr_data, variables, is_initial_timesteps, i, ind):
    ...
    losses['rigid'] = torch.tensor(0.0).cuda()
    losses['rot'] = torch.tensor(0.0).cuda()
    losses['iso'] = torch.tensor(0.0).cuda()
    losses['soft_col_cons'] = torch.tensor(0.0).cuda()
    losses['gray_col_cons'] = torch.tensor(0.0).cuda()
    ...
    if not is_initial_timesteps[1]:
        ...
        # losses['bg'] = l1_loss_v2(bg_pts, variables["init_bg_pts"]) + l1_loss_v2(bg_rot, variables["init_bg_rot"])
            ...
            # losses['uniform_rot'] =  l1_loss_v1(torch.nn.functional.normalize(params['unnorm_rotations']), variables['prev_rot'])

        # losses['gray_col_cons'] = separate_gray_loss_hsv(torch.sigmoid(params['rgb_colors']), variables["prev_col"], threshold=0.12) 
    ...
    loss_weights = {'im': 1.0*4, 'seg': 3.0*4, 'rigid': 4.0*weight, 'rot': 4.0*weight, 'iso': 2.0*weight, 'floor': 2.0, 'bg': 20.0,
            'soft_col_cons': 0.01*4,
            'gray_col_cons': 0.02,
            'cam_c':0.001,
            'cam_m':1,
            'smooth_scale': 0.1,
    ...
    variables['seen'] = seen
    return loss_components, variables
...
def initialize_post_first_timestep(params, variables, optimizer, num_knn=20):
    ...
    params_to_fix = ['logit_opacities', 'log_scales', 'cam_c', 'cam_m']
    # params_to_fix = ['log_scales', 'cam_m', 'cam_c']
...
def train(seq, exp, scale=1):
    ...
    criterion = WeightedLoss(num_losses=7)
    optimizer = initialize_optimizer(params, variables, criterion)
    ...
    ssim_lower_margin = 0.8 #0.001
    ssim_lower_margin2 = 0.8 #0.001
    ssim_init = 9999
    ...
    # for t in range(num_timesteps):
        ...
        running_loss = 0.0 
        with torch.no_grad():
            criterion.log_weights.fill_(0.0)
        while True:
        ...
        # for i in range(num_iter_per_timestep):
            ...
            try:
                ...
                loss_components, variables = get_loss(params, optimizer, curr_data, variables, is_key_timesteps, i, index )
                loss = criterion(loss_components)  
            ...
            with torch.no_grad():
                try:
                    ...
                    if len(ssim_values) > 0:
                        ...
                        progress_bar.set_postfix({"train img SSIM": f"{ssim_values[1]:.{7}f} , loss: {loss.cpu().numpy().item():.{7}f} "})
                        progress_bar.update(every_i)
                    else:
                        smallest_ssim = None
                    # if smallest_ssim is not None:
...
if __name__ == "__main__":
    ...
    # exp_name = "2025-05-20_18-21-50_tahir_2_fullsize_lumi_adj_aligned_start_60-450_interpolated_exp_1_test16"
    # for sequence in ["2025-05-20_18-21-50_tahir_2_fullsize_lumi_adj_aligned"]:

    exp_name = "2025-05-20_18-24-20_hamit_burak_1_lumi_adj_scl1_weight_test10"
    for sequence in ["2025-05-20_18-24-20_hamit_burak_1_lumi_adj"]:
