...
from helpers_scl import combined_color_loss_v2,combined_color_loss_v1, convert_rgb_to_hsv, l1_loss_v1_noblack, l2_loss_v1, l2_loss_v2, params2cpu2,  pytorch3d_knn, separate_gray_loss, separate_gray_loss_hsv, separate_gray_loss_hsv_smooth, separate_gray_loss_lab, separate_gray_loss_lab_smooth, setup_camera, l1_loss_v1, l1_loss_v2, weighted_l2_loss_v1, weighted_l2_loss_v2, quat_mult, \
    o3d_knn, params2rendervar, params2cpu, save_params, save_params_multi
from external import accumulate_mean2d_gradient, accumulate_mean3d_gradient, calc_ssim, calc_psnr, build_rotation, calc_ssim_with_mask, densify, densify_nokey_frames, inverse_sigmoid, keep_points, np_inverse_sigmoid, update_params_and_optimizer
import shutil
...
def accumulate_corrupted_points(params, dataset, variables, optimizer):
    ...
    for dt in dataset:
        with torch.no_grad():
            ...
            # mask_thick = keep_thick_regions(output.permute(1, 2, 0).cpu().numpy().astype(np.uint8)  )
            # output_thick = torch.zeros_like(diff , dtype=torch.uint8)
            # output_thick = output_thick.permute(1, 2, 0)
            # output_thick[mask_thick] = torch.tensor([255, 255, 255], dtype=torch.uint8, device=output_thick.device)
            # im_g = Image.fromarray(output_thick.cpu().numpy().astype(np.uint8))
            
            # img.save(f"mask_image_thick_{dt['id']}.png")
...
def get_loss(params, optimizer, curr_data, variables, is_initial_timestep, i, list_ids, vertices):
    ...
    if  i %3 == 0 and ( curr_id == 0 or  curr_id == 35):
        torchvision.utils.save_image(im, f'im3.png')
    ...
    # for t in range(num_timesteps):
        ...
        while True:
            ...
            try:
                ...
                # if i == 200  and not is_key_timestep:
            ...
            loss.backward()
            # if not is_key_timestep and i >= 200 and i % 100 == 0 :
            #     params = update_params(indices, params, optimizer, variables)
...
if __name__ == "__main__":
    ...
    # exp_name = "2025-05-20_18-21-50_tahir_2_fullsize_lumi_adj_aligned_test37"
    # for sequence in ["2025-05-20_18-21-50_tahir_2_fullsize_lumi_adj_aligned"]:

    # exp_name = "2025-08-06_16-07-58_combin2_test2"
    # for sequence in ["2025-08-06_16-07-58_combin2"]:

    # exp_name = "2025-08-06_16-09-24_3412x2500_combin2_test1"    
    # for sequence in ["2025-08-06_16-09-24_3412x2500_combin2"]:

    # exp_name = "2025-08-06_16-09-24_3412x2500_combin2_2_test1_start_118_3"    
    # for sequence in ["2025-08-06_16-09-24_3412x2500_combin2_2"]:
