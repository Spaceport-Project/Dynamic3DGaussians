...
def get_loss(params, curr_data, variables, is_initial_timestep, t):
    ...
    loss_weights = {'im': 1.0, 'seg': 0.5, 'rigid': 4.0, 'rot': 4.0, 'iso': 2.0, 'floor': 2.0, 'bg': 20.0,
                    'soft_col_cons': 0.01}
            ...
            with torch.no_grad():
                try:
                    ...
                    for id in range(num_cams):
                        psnr, ssim_value = report_progress(params, dataset[id], i + 1, progress_bar, every_i)
                        if psnr is not None :
...
if __name__ == "__main__":
    ...
    exp_name = "2025-05-20_18-21-50_tahir2_fullsize_scl_1_contrast_alpha14_aligned_half_test4"
