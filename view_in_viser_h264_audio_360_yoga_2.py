...
class Viewer():
    ...
    def __init__(self, seq, exp, pc_path, f_ratio=0.8, w=1920, h=1080, near=0.01, far=100.0):
        self.seq = seq
        self.exp = exp
        self.pc_path = pc_path
        self.viser_server = viser.ViserServer(port=8083)
...
class RenderViewers():
    ...
    look_at =  np.array([-3.41289393e-01,  1.79894763e+00,  1.27396714e-03])
    ...
    def crop_person2(image_tensor, mask):
        ...
        return cropped_image, (y_min, y_max, x_min, x_max)
    @staticmethod
    def resize_and_place2(image_tensor, cropped_tensor, bbox, scale=1.2):
    ...
    # Load the image and move it to GPU
        ...
        output_tensor = RenderViewers.resize_and_place(image_tensor, cropped_tensor, green_rgb, bbox, scale)
    ...
    def _render(self, c2w, timestep_data, gaussians, bg=[0,0,0], is_cam_pose_changed=True):
        with torch.no_grad():
            ...
            # c2w_clone[:3,3] *=0.8
            w2c = np.linalg.inv(c2w)
            cam = setup_camera(self.w, self.h, self.k, w2c, self.near*0.5, self.far*0.5, scale= 1., bg=torch.tensor(bg)) #[0, 177./255, 64.0/255]

            im, _, _, = Renderer(raster_settings=cam)(**timestep_data)
            # im = RenderViewers.process_image(im, bg, scale=0.7)
            # torchvision.utils.save_image(im, '{0:05d}'.format(1) + ".png")
...
if __name__ == "__main__":
    ...
    # sequence = "bicycle_2"
    point_cloud = "/home/hamit/gaussian-splatting/output/bicycle/point_cloud/iteration_30000/point_cloud.ply" 
        
    viewer = Viewer(seq=sequence, exp=exp_name,  pc_path= point_cloud, w=1920, h=1080)
    time.sleep(0.2)
    viewer.start_viewer()
