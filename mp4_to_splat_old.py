...
def parse_arguments():
    ...
    parser.add_argument(
        "--training_env_name",
        type=str,
        default="dynamic_gaussians",
        help="Conda environment name used for training (default: dynamic_gaussians)"
    )
    parser.add_argument(
...
def extract_frames(input_video, output_dir, frame_interval):
    ...
    try:
        ...
        logger.error(f"Frame extraction failed with code {e.returncode}")
        sys.exit(1)
...
def remove_backgrounds(input_dir, output_dir, model_name):
    ...
    try:
        ...
        logger.error("rembg not found. Please install rembg or add it to PATH")
        sys.exit(1)
...
def run_colmap_pipeline(source_path, image_path, colmap_executable, camera_model, use_gpu):
    ...
    feat_extraction_cmd = (
        ...
        f'--SiftExtraction.num_threads 32 '
        f'--SiftExtraction.domain_size_pooling true '
        f'--SiftExtraction.max_image_size 8000 '
        f'--SiftExtraction.max_num_features 11000 '
        f'--SiftExtraction.use_gpu {use_gpu_flag}'
    )
...
def run_training_with_conda(train_config_path, env_name="dynamic_gaussians"):
    """Run training script in a dedicated conda environment."""
    ...
    logger.info("=" * 70)
    logger.info("Starting training in separate conda environment...")
    ...
    try:
        ...
        logger.error("conda executable not found in PATH")
    ...
    if args.create_meta:
        ...
        if args.run_training:
            if train_config_path is None:
                ...
                run_training_with_conda(train_config_path, env_name=args.training_env_name)
