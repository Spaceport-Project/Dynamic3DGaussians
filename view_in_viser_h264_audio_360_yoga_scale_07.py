...
class RenderViewers():
    ...
    def _render(self, c2w, timestep_data, gaussians, bg=[0,0,0], is_cam_pose_changed=True):
        with torch.no_grad():
            ...
            # c2w_clone[:3,3] *=0.8
            w2c = np.linalg.inv(c2w)
            cam = setup_camera(self.w, self.h, self.k, w2c, self.near, self.far, scale= 1/0.7, bg=torch.tensor(bg)) #[0, 177./255, 64.0/255]
    ...
    #    [-0.1549435 ,  0.96627859,  0.20566528,  1.26594328],   # for 0.5 scale
    #    [ 0.68246103,  0.25521298, -0.68491844,  1.4389307 ],
    #    [ 0.        ,  0.        ,  0.        ,  1.        ]])
        

        c2w = np.array([[-0.67573547,  0.01495594, -0.73699247,  1.37058484],
       [-0.17505011,  0.96793903,  0.18014296,  1.38051446],
       [ 0.71605799,  0.2507396 , -0.65145269,  1.51445779],
       [ 0.        ,  0.        ,  0.        ,  1.        ]])
