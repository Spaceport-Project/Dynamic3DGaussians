...
def reduce_purple_tint(image, blue_factor=0.75, red_factor=0.75, green_boost=1.1):
    result = image.copy()
        ...
        try:    
            ...
            for bbox_data in bboxes_data:
                if (bbox_data["class_name"] == "person" or bbox_data["class_name"] == "dog") :
                    area = (bbox_data['bbox'][2] - bbox_data['bbox'][0]) *  (bbox_data['bbox'][3] - bbox_data['bbox'][1])
                    # print(area)
                    if area > 150000:
                        person_bboxes.append(bbox_data["bbox"])
                    
            person_bboxes_0 = np.array(sorted(person_bboxes, key= lambda box: (box[2] - box[0]) *  (box[3] - box[1]), reverse=True)[:2]) if len(person_bboxes) > 1 else np.array(sorted(person_bboxes, key= lambda box: (box[2] - box[0]) *  (box[3] - box[1]), reverse=True)[0])
            ...
            image_uncorr = cv2.imread(image_name)
            image = reduce_purple_tint(image_uncorr)
            ...
            for box  in person_bboxes:
                ...
                mask, _, _ = sam_predictor.predict(
                    box = np.array(box), #person_bboxes,#.cpu().numpy(),
                    multimask_output=False
            ...
            # pt1 = tuple(person_bboxes_0.astype(np.int32)[:2])
            # pt2 = tuple(person_bboxes_0.astype(np.int32)[2:4])
            # cv2.rectangle(rgba_image, pt1, pt2, (0, 255, 0), 5)
