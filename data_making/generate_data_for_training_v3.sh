...
find_matching_timestamp(){
    local arr1=()
    local arr2=()

    local first_timestamp=$(find $dataset_path  -type f -name "Cam_*.png" | sort -V |  awk '{n=split($1,A,"_"); print A[n-1]}' | sort -V  | uniq -c | awk -v cam_number="${cam_number}" '$1 == cam_number {print $2; exit}')
    arr1+=($first_timestamp)
    local first_timestamp_full=$(find $dataset_path  -type f -name "Cam-Full_*.png" | sort -V |  awk '{n=split($1,A,"_"); print A[n-1]}' | sort -V  | uniq -c | awk -v cam_number="${cam_number}" '$1 == cam_number {print $2; exit}')
    arr1+=($first_timestamp)
    if [ "${first_timestamp}" != "${first_timestamp_full}" ]
