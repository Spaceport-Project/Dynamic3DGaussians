...
calculate_number_valid_timesteps() {
    ...
    for idx in  "${!list_timestamps[@]}"; do
        ...
        cam_number_minus_one=$((cam_number))
    ...
    for idx in  "${!list_valid_ts[@]}"; do
        ...
        for sn in "${list_serial_numbers[@]}"; do
            ...
            if [ -e "$file" ]; then 
                ...
                mv  $file  $output_folder_path/${folder_cam}/$a/$formatted_number.${ext}
                ...
                for it in "${!iters[@]}"; do
                    ...
                    if [ -e "$next_file" ]; then 
                        ...
                        mv  $next_file  $output_folder_path/${folder_cam}/$a/$formatted_number.${ext}
                        break 
                    fi
                    kk=$((kk+1))
                    if [ "$kk" -eq "${#iters[@]}" ]; then
...
run_colmap(){
    ...
    for file in `find $ims_path   -name "000000.${ext}" | sort -V`;
    do
