...
def organize_images_folder_by_cam(list_valid_ts, dataset_path, output_folder_path, cam_folder,
...
def run_colmap_undistort_mask_frames(input_folder_path, number_timesteps, cam_folder, output_fol_name, seg_output_fol_name, ext ="png"):
    """
    Python version of run_colmap_undistort_frames bash function
    ...
    if not os.path.isdir(input_folder_path):
        raise FileNotFoundError(f"Input path not found: {input_folder_path}")
    

    
    
    # Calculate processes per GPU (you can adjust this)
    processes_per_gpu = 1  # 6 processes per GPU = 18 total processes
...
if __name__ == '__main__':
    ...
    number_timesteps=  len(valid_timestamps)
    ...
    run_colmap_undistort_mask_frames(
