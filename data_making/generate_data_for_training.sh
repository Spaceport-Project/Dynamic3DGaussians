...
organize_images_folder_by_cam(){
    ...
    local id=0
    for idx in  "${!list_valid_time_stamps[@]}"; do
        local iters=(${list_valid_time_stamps[$idx+1]} ${list_valid_time_stamps[$idx+2]} ${list_valid_time_stamps[$idx+3]} ${list_valid_time_stamps[$idx+4]} ${list_valid_time_stamps[$idx+5]} ${list_valid_time_stamps[$idx+6]})
        ...
        for sn in "${list_serial_numbers[@]}"; do
            ...
            local file="$dataset_path/frame_${list_valid_time_stamps[$idx]}_$sn.png"
            if [ -e "$file" ]; then 
        ...
        echo "Copying images from $a cameras for ${list_valid_time_stamps[$idx]} timestamp has finished!" >&2
        id=$((id+1))
    done
...
run_masking_command() {
    ...
    rm -rf $output_folder_path/$masked_folder
