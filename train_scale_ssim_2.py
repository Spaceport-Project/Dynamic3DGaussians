...
def get_loss(params, curr_data, variables, is_initial_timestep, i):
    ratio = 0.80
    losses = {}
    ...
    if not is_initial_timestep:
        ...
        # color_diff = l1_loss_v2(params['rgb_colors'], variables['prev_col'])

        losses['soft_col_cons'] =  l1_loss_v2(params['rgb_colors'], variables["prev_col"])



        # Weight by opacity - more important colors get higher weight
        opacity_weights = params['logit_opacities'].sigmoid()
        # weighted_color_diff = (color_diff * opacity_weights.detach()).mean()
        # losses['enhanced_col_temporal'] = weighted_color_diff

        # ema_decay = 0.9
        # variables['color_ema'] = ema_decay * variables['color_ema'] + \
        #                    (1 - ema_decay) * params['rgb_colors']
        # losses['color_ema_cons'] = l1_loss_v2(params['rgb_colors'], variables['color_ema'])  
        ...
        # losses['opacity_temporal'] = l1_loss_v2(params['logit_opacities'], variables['prev_logit_opacities'])
    
    loss_weights = {'im': 1.0, 'seg': 2.0, 'rigid': 1.0, 'rot': 1.0, 'iso': 2.0, 'floor': 2.0, 'bg': 20.0,
                    'soft_col_cons': 3, 'enhanced_col_temporal': 0.03, 'color_ema_cons': 0.02} 
    # if i >= 300 and not is_initial_timestep:
    #     loss_weights = {'im': 0.0, 'seg': 0.0, 'rigid': 0.0, 'rot': 0.0, 'iso': 0.0, 'floor': 0.0, 'bg': 0.0,
    #                 'soft_col_cons': 5, 'enhanced_col_temporal': 5, 'color_ema_cons': 2} 
...
def initialize_per_timestep(params, variables, optimizer):
    ...
    variables['color_ema'] = params['rgb_colors'].detach().clone()  

    # variables["prev_logit_opacities"] = params['logit_opacities'].detach()
            ...
            with torch.no_grad():
                ...
                if smallest_ssim is not None:
                    ...
                    if  is_key_timestep == False and  i == num_iter_per_timestep - 1 and smallest_ssim  < ssim_init - ssim_lower_margin :
                        ...
                        if num_iter_per_timestep > num_iter_per_timestep_for_non_key_frames * 6:
...
if __name__ == "__main__":
    ...
    # exp_name = "2025-05-20_18-21-50_tahir_2_half_all_low_error_scl_1_enhanced_1163"
    # for sequence in ["2025-05-20_18-21-50_tahir_2_half_all_low_error"]:

    exp_name = "2025-05-20_18-24-20_hamit_burak_1_half_all_scl_1_enhanced_60_flicker_test7"
    ...
    # for sequence in ["2025-05-20_18-24-20_hamit_burak_1_half_24cams"]:
