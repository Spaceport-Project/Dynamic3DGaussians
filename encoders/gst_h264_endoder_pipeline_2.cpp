...
void push_tensor_frame(uintptr_t data_ptr, size_t  size, unsigned int frame_number, unsigned int frame_rate, int cnt) {
    // Create a GstBuffer that wraps the existing image data
    
    // uint8_t *data_ptr_new =(uint8_t *)data_ptr;
    uint8_t *data_ptr_new = (uint8_t *)malloc(size);
    memcpy(data_ptr_new,  (uint8_t *)data_ptr, size);
    // free((uint8_t *)data_ptr);
    // cudaMemcpy(data_ptr, (uint8_t *)cuda_ptr, size, cudaMemcpyDeviceToHost);
    // std::cout<<"size in c++:"<<size<<std::endl;
    
    GstBuffer *buffer = gst_buffer_new_wrapped((uint8_t *)data_ptr_new, size);
    // GstBuffer *buffer = gst_buffer_new_wrapped_full(
    //     GST_MEMORY_FLAG_READONLY,  // Memory flags
    //     (uint8_t *)data_ptr_new,       // Data pointer
    //     size,                      // Total size
    //     0,                         // Offset
    //     size,                      // Maxsize
    //     NULL,                      // User data for free function
    //     NULL                       // Free function (NULL because we don't want GStreamer to free the memory)
    // );
...
static GstFlowReturn on_new_video_sample(GstElement *sink, gpointer data) {
    try {
        ...
        if (apps.find(cnt) == apps.end() || !apps[cnt]) {
        ...
        std::cerr << "Exception in on_new_video_sample: " << e.what() << std::endl;
        return GST_FLOW_ERROR;
    } catch (...) {
        std::cerr << "Unknown exception in on_new_video_sample" << std::endl;
        return GST_FLOW_ERROR;
    }
}
// static GstFlowReturn on_new_video_sample(GstElement *sink, gpointer data) {
...
int main_fun() {
    ...
    + "/1 !  videoconvert ! video/x-raw,format=NV12 ! nvh264enc gop-size=20 bframes=0 preset=low-latency-hq zerolatency=truebitrate=" + std::to_string(bitrate) + \
...
PYBIND11_MODULE(gst_h264_endoder_pipeline_2, m)
{
