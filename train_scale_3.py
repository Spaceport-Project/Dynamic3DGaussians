...
from helpers_new import setup_camera, l1_loss_v1, l1_loss_v2, weighted_l2_loss_v1, weighted_l2_loss_v2, quat_mult, \
...
def initialize_params_from_prev(seq, md, params_prev):
...
def train(seq, exp, scale=1):
    ...
    dataset_id = 1
    num_iter_per_timestep_for_key_frames = 10000
    step_num_iter_per_timestep_for_key_frames = 1000
    num_iter_per_timestep_for_non_key_frames = 700
    limit_num_iter_per_timestep_for_key_frames = 20000
    psnr_lower_limit = 39
    psnr_lower_margin = 0.7
    psnr_init = 999
    ...
    # for t in range(num_timesteps):
        ...
        # for i in range(num_iter_per_timestep):
            ...
            try:
                loss, variables = get_loss(params, curr_data, variables, is_key_timestep)
            ...
            with torch.no_grad():
                try:
                    psnr = report_progress(params, dataset[dataset_id], i + 1, progress_bar)
                ...
                if psnr is not None:
                    # if psnr_init < psnr_lower_limit and is_key_timestep == False and  count_non_key_frames > 2 :
                    #     psnr_init = psnr_lower_limit
                    #     params, variables = initialize_params_from_prev(seq, md, params) 
                    #     optimizer = initialize_optimizer(params, variables)
                    #     is_key_timestep = True
                    #     i = 0
                    ...
                    if  is_key_timestep == False and  i == num_iter_per_timestep - 1 and psnr  < psnr_init - psnr_lower_margin :
                        ...
                        if num_iter_per_timestep > num_iter_per_timestep_for_non_key_frames * 2:
                            ...
                            # variables = variables_cloned
                            # optimizer = optimizer_cloned
                            is_key_timestep = True
                            i = 0
                            num_iter_per_timestep = num_iter_per_timestep_for_key_frames
                            progress_bar = tqdm(range(num_iter_per_timestep), desc=f"timestep {t}")
                    ...
                    if is_key_timestep == False and i >= num_iter_per_timestep_for_non_key_frames and psnr  >= psnr_init - psnr_lower_margin:
                        break

                    if is_key_timestep == True and i == num_iter_per_timestep - 1 and num_iter_per_timestep < limit_num_iter_per_timestep_for_key_frames and psnr < psnr_lower_limit:
                        num_iter_per_timestep += step_num_iter_per_timestep_for_key_frames
                        progress_bar = tqdm(range(num_iter_per_timestep), desc=f"timestep {t}")
                    if is_key_timestep == True and i > num_iter_per_timestep_for_key_frames and psnr >= psnr_lower_limit:
                        psnr_init = psnr
                        break
                    if is_key_timestep == True and num_iter_per_timestep >= limit_num_iter_per_timestep_for_key_frames:
                        ...
                        # optimizer = initialize_optimizer(params, variables)
                        # skip_to_next_timestamp = True
                        # # dataset_id = (dataset_id+1)%24
                        # break
        ...
        if t in save_iterations:
            save_params_multi(output_params, seq, exp, index_key_timestep)
...
if __name__ == "__main__":
    ...
    exp_name = "2024-12-04_17-14-42_8bestcams_scl_2_test1_start_58_2"
    for sequence in ["2024-12-04_17-14-42_8bestcams"]:
