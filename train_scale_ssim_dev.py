...
def get_loss(params, curr_data, variables, is_initial_timestep, t, ind):
    ratio = 0.80
    losses = {}
    ...
    if not is_initial_timestep:
        ...
        # losses['gray_col_cons'] = separate_gray_loss_lab_smooth(torch.sigmoid(params['rgb_colors']), variables["prev_col"], variables["neighbor_indices"], threshold=10) 
        # else:
        #     del losses['seg']
        #     del losses['im']
            # losses['soft_col_cons'] = l1_loss_v1(torch.sigmoid(params['rgb_colors']), variables["prev_col"])
            losses['gray_col_cons'] = separate_gray_loss_hsv(torch.sigmoid(params['rgb_colors']), variables["prev_col"], threshold=0.1) 
            # losses['cam_m'] = l1_loss_v1(params['cam_m'], variables['cam_m'])
            # losses['cam_c'] = l1_loss_v1(params['cam_m'], variables['cam_c'])

        # losses['combined_col_cons'] = combined_color_loss_v2(torch.sigmoid(params['rgb_colors']), variables["prev_col"], factor=5.0)
        
        # rgb_colors = rendervar['colors_precomp']
        # rgb_colors = rgb_colors[variables["neighbor_indices"]]
        # losses['soft_col_cons'] =  weighted_l2_loss_v2(rgb_colors, variables["prev_col"][:,None], variables["neighbor_weight"])

    #    # Position temporal smoothness (NEW)
    #     losses['temporal_smooth_pos'] = l1_loss_v2(params['means3D'], variables["prev_pts"])

    #     # Rotation temporal smoothness (NEW)
    #     curr_rot = torch.nn.functional.normalize(params['unnorm_rotations'])
    #     losses['temporal_smooth_rot'] = l1_loss_v2(curr_rot, variables["prev_rot"])

        # Scale temporal smoothness (NEW)
        # losses['temporal_smooth_scale'] = l2_loss_v2(torch.exp(params['log_scales']), variables["prev_scales"])

        # Opacity temporal smoothness (NEW)
        # losses['temporal_smooth_opacity'] = l1_loss_v1(torch.sigmoid(params['logit_opacities']), variables["prev_opacities"])
        
        # VELOCITY-BASED REGULARIZATION
        # current_velocity = params['means3D'] - variables["prev_pts"]
        # if 'prev_velocity' in variables:
        #     velocity_change = current_velocity - variables['prev_velocity']
        #     losses['velocity_consistency'] = torch.mean(velocity_change ** 2)
        # variables['prev_velocity'] = current_velocity.detach()
    # if (not is_initial_timestep) and  t >= 1000:
    #     loss_weights = {'im': 0.0, 'seg': 0.0, 'rigid': 0.0, 'rot': 0.0, 'iso': 0.0, 'floor': 0.0, 'bg': 0.0,
    #             'soft_col_cons': 1.0,
    #             'gray_col_cons': 1.0,
    #             # 'combined_col_cons': 0.0,
    #             # 'temporal_smooth_scale': 0.01,
    #             # 'temporal_smooth_opacity': 0.005,
    #     }
    # else:
    loss_weights = {'im': 1.0, 'seg': 3.0, 'rigid': 4.0*5, 'rot': 4.0*5, 'iso': 2.0*5, 'floor': 2.0, 'bg': 20.0,
            'soft_col_cons': 0.01,
            'gray_col_cons': 0.05,
            'cam_c':0.001,
            'cam_m':0.01,
            # 'combined_col_cons': 0.01,
            # 'temporal_smooth_scale': 0.01,
            # 'temporal_smooth_opacity': 0.005,
...
def initialize_per_timestep(params, variables, optimizer, alpha=0.7):
    ...
    # if 'prev_prev_pts' in variables:
    #     # Use smooth velocity estimation
    #     velocity = alpha * (pts - variables["prev_pts"]) + (1-alpha) * (variables["prev_pts"] - variables['prev_prev_pts'])
    #     new_pts = pts + velocity

    #     # Update prev_prev values for next iteration
    #     variables['prev_prev_pts'] = variables["prev_pts"].detach()  # UPDATE HERE
    #     variables['prev_prev_rot'] = variables["prev_rot"].detach()
    # else:
    #     # This shouldn't happen if initialize_post_first_timestep ran
    #     new_pts = pts + (pts - variables["prev_pts"])
...
def update_some_params(params, optimizer, variables):
    ...
    params_to_fix = ['means3D', 'unnorm_rotations', 'cam_m', 'cam_c']
...
def initialize_post_first_timestep(params, variables, optimizer, num_knn=20):
    ...
    # variables["prev_scales"] = params['log_scales'].detach()  
    # variables["prev_opacities"] = params['logit_opacities'].detach()  
    ...
    # variables["cam_m"]= params['cam_m'].clone().detach()
    # variables["cam_c"]= params['cam_c'].clone().detach()
    #   # Initialize prev_prev variables for second-order smoothing  
    # variables["prev_prev_pts"] = params['means3D'].detach()  
    # variables["prev_prev_rot"] = torch.nn.functional.normalize(params['unnorm_rotations']).detach()  

    # # Initialize prev_velocity (ADD THIS)
    # variables["prev_velocity"] = torch.zeros_like(params['means3D']).detach()
...
def train(seq, exp, scale=1):
    ...
    # for t in range(num_timesteps):
        ...
        # for i in range(num_iter_per_timestep):
            ...
            try:
                loss, variables = get_loss(params, curr_data, variables, is_key_timestep, i, index )
            ...
            with torch.no_grad():
                try:
                    ...
                    if len(ssim_values) > 0:
                        ...
                        progress_bar.set_postfix({"train img 0 SSIM": f"{ssim_values[1]:.{7}f}"})
                        progress_bar.update(every_i)
                    else:
                        smallest_ssim = None
                ...
                if smallest_ssim is not None:
                    ...
                    if  is_key_timestep == False and  i == num_iter_per_timestep - 1 and smallest_ssim  < ssim_init - ssim_lower_margin :
                        ...
                        if num_iter_per_timestep > num_iter_per_timestep_for_non_key_frames * 3:
                    ...
                    if is_key_timestep == False and i >= num_iter_per_timestep_for_non_key_frames - 1 and i < num_iter_per_timestep_for_non_key_frames * 4 and smallest_ssim  >= ssim_init - ssim_lower_margin:
                        every_i = every_big
                        ssim_lower_limit_var = ssim_lower_limit
...
if __name__ == "__main__":
    ...
    # exp_name = "2025-05-20_18-21-50_tahir_2_lumi_adj_scl1_enhanced_test10"
