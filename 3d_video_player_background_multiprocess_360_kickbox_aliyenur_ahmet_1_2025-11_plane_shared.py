...
class Viewer():
    ...
    def load_list_params_files(self, seq, exp):
        params_files =[ os.path.join(f"./output/{exp}/{seq}/", file) for file in  os.listdir(f"./output/{exp}/{seq}/") if file.startswith("batch")]
        ...
        for l, params_file  in enumerate(params_files):
            ...
            #         #     continue
            #         length = params["means3D"].size
            #         # try:
            #         #     data = params[param]
            #         # except Exception as e:
            #         #     print(f"{params_file} has problem")
            #         #     sys.exit(1)
            #         # length= 0
            #         # for pr in data:
            #         #     length += len(pr)
    ...
    def load_scenes(self):
        ...
        while self.running.value:
            ...
            for interval, params_file in self.params_files_dict.items():
                ...
                with h5py.File(params_file, 'r') as f:
                    ...
                    while cnt < len(f["means3D"]):
                        ...
                        with torch.no_grad():
                            ...
                            rendervar = {
                                'means3D': torch.tensor(f['means3D'][str(cnt)], dtype=torch.float32),
                                'colors_precomp': torch.sigmoid(torch.tensor(f['rgb_colors'][str(cnt)], dtype=torch.float32)),
                                'rotations': torch.nn.functional.normalize(
                                    torch.tensor(f['unnorm_rotations'][str(cnt)], dtype=torch.float32)
                                ),
                                'opacities': torch.sigmoid(
                                    torch.tensor(f['logit_opacities'][str(cnt)], dtype=torch.float32)
                                ),
                                'scales': torch.exp(
                                    torch.tensor(f['log_scales'][str(cnt)], dtype=torch.float32)
                                ),
                                'means2D': torch.zeros_like(
                                    torch.tensor(f['means3D'][str(cnt)], dtype=torch.float32)
                                )
                            }
                            # print("timestep:",t )
                        print(cnt, t, time.time()-t1)
                ...
                #         rendervar = {
                #             'means3D': torch.tensor(params['means3D'][cnt], dtype=torch.float32),
                #             'colors_precomp': torch.sigmoid(torch.tensor(params['rgb_colors'][cnt], dtype=torch.float32)),
                #             'rotations': torch.nn.functional.normalize(
                #                 torch.tensor(params['unnorm_rotations'][cnt], dtype=torch.float32)
                #             ),
                #             'opacities': torch.sigmoid(
                #                 torch.tensor(params['logit_opacities'][cnt], dtype=torch.float32)
                #             ),
                #             'scales': torch.exp(
                #                 torch.tensor(params['log_scales'][cnt], dtype=torch.float32)
                #             ),
                #             'means2D': torch.zeros_like(
                #                 torch.tensor(params['means3D'][cnt], dtype=torch.float32)
                #             )
                #         }
                #             # print("timestep:",t )
                #     print(cnt, t, time.time()-t1)
...
if __name__ == "__main__":
    ...
    exp_name = "2025-11-12_14-37-30_aliyenur_ahmet_1_50-1850_0_combined"
    sequence = "2025-11-12_14-37-30_aliyenur_ahmet_1_50-1850"
    ...
    viewer = Viewer(seq=sequence, exp=exp_name, pc_path=pc_path, port=8094, title="360° Demo8 Spaceport", w=1920, h=1080, max_sh_degree=3)
